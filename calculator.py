"""Calculate information and Graph info."""
...
RDI_PERCENTS = {'carbs': 0.4, 'fats': 0.3,
                'protein': 0.3}  # constant variable
...
class CalcFunc:
    ...
    def calculation_and_graph(self):
        ...
        if all(meal_selected.get() == "NO SELECTION/SKIP"
            ...
            if self.charts is not None:  # if theres a open chart window
                self.charts.get_tk_widget().destroy()  # destroy open window
        ...
        try:  # safety net for ANY unexpected errors during calculation
            ...
            for nutrient in nutrients:  # for each nutrient in the list
                ...
                for menu, selection_var in zip(
                        self.all_menus, self.selected_meals):  # for each
                    nutrient_value += menu[
                        selection_var.get()][nutrient]  # get nut info in meal
                nutrient_totals[nutrient] = nutrient_value  # updates value
        ...
        except Exception:  # if an unexpected/unknown error occurs
            self.results.config(text="Calculation error.\nPlease try again.")
            return None  # stops the function

        # ---CHARTS AND GRAPHS START HERE---
        try:  # safety net for graphing errors
            fig, axs = plt.subplots(1, 3, figsize=(10, 5),
                                    facecolor="#f0f0f0")
            fig.canvas.manager.set_window_title('Macro Charts and Graphs')

            # FIRST CHART (CALORIES CONSUMED/REMAINING)
            if total_calories <= MAX_CALORIES:  # if user doesn't exceed cals
            ...
            else:  # if user exceeds recommended calories
                exceeded = total_calories - MAX_CALORIES
                pie_one_labels = [f'Allowed: {MAX_CALORIES} kcal',
            ...
            axs[2].bar(x - width / 2,
                       macronutrients, width, label='Total Consumed')
            axs[2].bar(x + width / 2, rdi_macros, width,
            ...
            plt.tight_layout()
            if self.charts is not None:  # If there's a chart that's running
                self.charts.get_tk_widget().destroy()  # destroy that chart
                self.charts = None  # once destroyed, set value to None
            self.charts = fig
        ...
        except Exception:  # if an unknown error occurs during graphing
            self.results.config(text="Graphing error.\nPlease try again.")
